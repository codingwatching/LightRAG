...
interface ImportMetaEnv {
   ...
   *
   * Must equal the backend `LIGHTRAG_API_PREFIX` (the prefix the reverse
   * proxy strips before forwarding to FastAPI). Empty / `/` → no prefix.
   *
   * Statically replaced into the bundle at `bun run build` time. See
   * `lightrag_webui/.env.example` and the project root `env.example`.
   *
